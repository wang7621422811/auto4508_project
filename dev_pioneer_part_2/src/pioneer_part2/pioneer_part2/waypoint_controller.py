"""
Enhanced waypoint controller for Pioneer P3-AT Part 2.

Extends the Part 1 controller with:
  - Wider position tolerance (1.5 m) to match task spec ("within 1-2 metres")
  - Pass-on-right behaviour: when within approach range, biases angular command
    so the waypoint cone stays to the robot's right side
  - Publishes /waypoint_reached (std_msgs/Int32) when each waypoint is reached
  - Accepts /mission_active (Bool) to pause driving when mission is idle

State machine
-------------
IDLE -> ROTATE_TO_GOAL -> DRIVE (with inline avoidance + pass-on-right) -> ROTATE_TO_HEADING
      <- next waypoint <--------------------------------------------------/
"""
...
class WaypointController(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter('position_tol', 1.5)
        ...
        self.declare_parameter('scan_front_angle', 0.7)
        # How far from a waypoint to start biasing for pass-on-right (m)
        self.declare_parameter('pass_on_right_range', 3.0)
        # Angular bias magnitude (rad) to keep cone on right
        self.declare_parameter('pass_on_right_offset', 0.35)
        ...
        self._pass_range = self.get_parameter('pass_on_right_range').value
        self._pass_offset = self.get_parameter('pass_on_right_offset').value
        ...
        self._reached_pub = self.create_publisher(Int32, 'waypoint_reached', 10)
        ...
        self.create_subscription(Bool, 'mission_active', self._active_cb, 10)
        ...
        self.get_logger().info('Waypoint controller (Part 2) ready')
    ...
    def _waypoints_cb(self, msg: Path):
        ...
        self.get_logger().info(f'Received path with {self._wp_total} waypoints')
        if self._queue:
            self._advance()
        self._publish_path_viz()
    ...
    def _control_loop(self):
        ...
        # ------ Phase 2: drive + reactive avoidance + pass-on-right ------
            ...
            # Pass-on-right: add a positive (left-turn) bias so the cone
            # ends up on the robot's right as we pass it.
            pass_bias = 0.0
            if dist < self._pass_range:
                # Bias increases as we get closer (scaled by remaining distance)
                scale = max(0.0, 1.0 - dist / self._pass_range)
                pass_bias = self._pass_offset * scale
            ...
            if min_front < self._obs_stop:
                ...
                if min_left <= min_right:
                    steer = self._kp_ang * heading_err - avoid_ang + pass_bias
                else:
                    steer = self._kp_ang * heading_err + avoid_ang + pass_bias
                ...
                cmd.linear.x = self._clamp(self._kp_lin * dist, self._max_lin)
                cmd.angular.z = self._clamp(
                    self._kp_ang * heading_err + pass_bias, self._max_ang)
        ...
        # ------ Phase 3: rotate to match goal heading ------
            ...
            if abs(orient_err) < self._hdg_tol:
                self._cmd_pub.publish(Twist())
                self.get_logger().info(
                    f'[{self._wp_index}/{self._wp_total}] Waypoint reached  '
                    f'pos_err={dist:.3f} m  yaw_err={math.degrees(orient_err):.1f}°')
                # Notify mission controller
                msg_idx = Int32()
                msg_idx.data = self._wp_index
                self._reached_pub.publish(msg_idx)
                self._advance()
                self._publish_path_viz()
                return
            cmd.angular.z = self._clamp(
